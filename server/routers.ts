...
export const appRouter = router({
  ...
  backup: router({
    export: protectedProcedure.query(async () => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [itemRows, purchaseRows, stockRows, orderRows, saleRows, shopRows, recipeRows, recipeLineRows, saleShopLineRows, adjustmentRows] = await Promise.all([db.select().from(items), db.select().from(purchases), db.select().from(dailyStock), db.select().from(orders), db.select().from(sales), db.select().from(shops), db.select().from(recipes), db.select().from(recipeLines), db.select().from(saleShopLines), db.select().from(stockAdjustments)]); return { schemaVersion: 1, exportedAt: new Date().toISOString(), items: itemRows, purchases: purchaseRows, dailyStock: stockRows, orders: orderRows, sales: saleRows, shops: shopRows, recipes: recipeRows, recipeLines: recipeLineRows, saleShopLines: saleShopLineRows, stockAdjustments: adjustmentRows }; }),
  }),
  costing: router({
