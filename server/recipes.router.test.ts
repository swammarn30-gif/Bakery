...
const state = { recipes: [] as Array<Record<string, unknown>>, recipeLines: [] as Array<Record<string, unknown>>, targetId: 2 };
...
vi.mock("./db", () => ({
  ...
  getDb: vi.fn(async () => ({
    ...
    delete: () => ({ where: async () => { state.recipeLines = []; } }),
...
describe("recipes.setActive router behavior", () => {
  beforeEach(() => { state.recipes = [{ id: 1, itemId: 10, active: true, effectiveFrom: "2026-01-01", note: "old" }, { id: 2, itemId: 10, active: false, effectiveFrom: "2026-02-01", note: "old" }, { id: 3, itemId: 11, active: true, effectiveFrom: "2026-01-01", note: null }]; state.recipeLines = []; state.targetId = 2; });
