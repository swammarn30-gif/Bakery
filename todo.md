...
- [ ] Fix the remaining post-authentication session handoff: Supabase sign-in succeeds, but the app returns to Sign In after `auth.me`; trace token attachment and Worker bearer verification, then redeploy and retest session persistence.
- [ ] Add narrowly scoped diagnostics for Supabase bearer verification and `auth.me` context failures, excluding token/password values; reproduce once, inspect Cloudflare logs, then remove diagnostics or replace them with the evidence-based minimal fix.
