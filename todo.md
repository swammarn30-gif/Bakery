...
- [x] Fix the verified auth race in `useAuth`: `auth.me` is now gated on `sessionReady`, with focused regression coverage. TypeScript, 56 Vitest tests, production build, and Wrangler dry-run pass; Cloudflare redeploy and final live verification remain pending.
