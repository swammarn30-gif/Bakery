...
- [ ] Fix automatic carry-forward so correcting a prior day through the Website UI updates later-day automatic Opening values to the prior Closing, then verify this live before resuming later-date correction.
- [ ] Fix the daily-ledger multi-field autosave path so a Website UI row edit persists the complete latest draft rather than reverting on a date reload.
